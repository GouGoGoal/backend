...
[Service]
Type=simple
ExecStartPre=iptables -t nat -A OUTPUT -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53
ExecStart=/usr/sbin/smartdns -f -c /etc/smartdns.conf
ExecStopPost=iptables -t nat -D OUTPUT -p udp --dport 53 -j DNAT --to-destination 127.0.0.1:53
Restart=always
